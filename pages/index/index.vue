		...
		<view :class="{dpn: isShowSearch}">
			<uni-nav-bar fixed :status-bar="true" @clickRight="goPlay">
				<block slot="left">
					 <block>
						<!-- #ifdef MP-WEIXIN -->
						<image src="/static/image/mine/r.png"  class="top-img" @click="goPlay"></image>
						<!-- #endif -->
						<!-- #ifdef MP-WEIXIN -->
						<image src="/static/image/search/6.png"  class="top-img p110" @click="goCloud"></image>
						<!-- #endif -->
					</block>
				</block>
				<view class="top-search flex-box" @click="openSearch">
					<image src="/static/image/search/2.png" class="search-icon"></image>
					{{searchText}}
				</view>
				<!-- #ifdef APP-PLUS ||H5 -->
				...
				<mescroll-uni ref="mescroll" :fixed="false" :down="downOption" :up="upOption" @down="downCallback" @up="upCallback">
					<view class="banner">
						<swiper :indicator-dots="true" indicator-color="rgba(255,255,255,.5)"
						 :autoplay="true"  indicator-active-color="#ff372b" 
...
<script>
	...
	import { apiGetBanner, apiGetTopAlbum,apiGetRelatedVideo,apiGetHotList } from '@/apis/index1.js';
